...
| BS_Pair_Par :
  forall env rho ea1 ef1 ea2 ef2 v1 v2 theta1 theta2
    ...
    (heap, env, rho, Pair_Par ef1 ea1 ef2 ea2) ⇓ (heap', Pair (v1, v2),
        Phi_Seq (Phi_Par acts_eff1 acts_eff2) (Phi_Par acts_mu1 acts_mu2))

| BS_Cond_True :
  forall e et ef env rho v (heap cheap theap : Heap) (cacts tacts : Phi),
    (heap, env, rho, e) ⇓ (cheap, (Bit true), cacts) ->
    (cheap, env, rho, et) ⇓ (theap, v, tacts) ->
    (heap, env, rho, Cond e et ef) ⇓ (theap, v, Phi_Seq cacts tacts)

| BS_Cond_False :
  forall e et ef env rho v (heap cheap fheap : Heap) (cacts facts : Phi),
    (heap, env, rho, e) ⇓ (cheap, (Bit false), cacts) ->
    (cheap, env, rho, ef) ⇓ (fheap, v, facts) ->
    (heap, env, rho, Cond e et ef) ⇓ (fheap, v, Phi_Seq cacts facts)

| BS_New_Ref :
  forall e w r l v env rho  (heap heap' : Heap) vacts,
    (heap, env, rho, e) ⇓ (heap', v, vacts) ->
    find_R w rho = Some r ->
    (*find_H (r, l) heap' = None -> *)
    allocate_H heap' r = l ->
    (heap, env, rho, Ref w e) ⇓ (update_H ((r, l), v) heap', Loc (Rgn2_Const true false r) l,
...
| BS_Set_Ref :
  forall ea ev w r l v env rho (heap heap' heap'' : Heap) (aacts vacts : Phi),
    (heap, env, rho, ea) ⇓ (heap', Loc w l, aacts) ->
...
| BS_Nat_Plus :
  forall a b va vb env rho (heap lheap rheap : Heap) (lacts racts : Phi),
...
| BS_Read_Conc :
  forall ea r l env rho (heap heap' : Heap) aacts,
    ...
    aacts = Phi_Nil ->
    (heap, env, rho, ReadConc ea) ⇓ (heap', Eff (Some (singleton_set (CA_ReadConc r l))),
        Phi_Nil)
...
| BS_Write_Conc :
  forall ea r l env rho (heap heap' : Heap) aacts,
    (heap, env, rho, ea) ⇓ (heap', Loc (Rgn2_Const true false r) l, aacts) ->
    aacts = Phi_Nil ->
    (heap, env, rho, WriteConc ea) ⇓ (heap', Eff (Some (singleton_set (CA_WriteConc r l))),
        Phi_Nil)
