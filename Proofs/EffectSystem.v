...
Proof.
  ...
  Case "mu_app".
    ...
    assert (Sf : Epsilon_Phi_Soundness (fold_subst_eps rho efff, facts))
    ...
    rewrite <- H11; eapply IHD3 with (stty := sttya) (rho:=rho'); eauto.
    SCase "Extended Env".
      apply update_env.
      SSCase "TcEnv".
        { apply update_env.
  ...
  Case "rgn_app".
    ...
    replace (Rgn_Const true true v') with  (mk_rgn_type (Rgn_Const true false v'))
  ...
  Case "par_pair".
    ...
    eapply IHD2; eauto.  
    assert (H_ : Epsilon_Phi_Soundness
                   (Union_Static_Action (fold_subst_eps rho eff1)
                      (fold_subst_eps rho eff2), Phi_Seq acts_mu1 acts_mu2))
     by (apply sound_comp; auto).
    assert (H__ : Epsilon_Phi_Soundness
                    (Union_Static_Action (fold_subst_eps rho eff3)
                       (fold_subst_eps rho eff4), Phi_Seq acts_eff1 acts_eff2))
     by (apply sound_comp; auto).

    rewrite fold_dist_union.
    replace (fold_subst_eps rho (Union_Static_Action
                                   (Union_Static_Action eff3 eff4) eff2)) with
      (Union_Static_Action
         (fold_subst_eps rho (Union_Static_Action eff3 eff4)) (fold_subst_eps rho eff2))
      by (rewrite <- fold_dist_union; reflexivity).

    replace (Union_Static_Action (Union_Static_Action
                                    (fold_subst_eps rho (Union_Static_Action eff3 eff4))
                                    (fold_subst_eps rho eff2))
               (fold_subst_eps rho eff1)) with
     (Union_Static_Action (fold_subst_eps rho (Union_Static_Action eff3 eff4)) 
        (Union_Static_Action (fold_subst_eps rho eff1)
           (fold_subst_eps rho eff2))).
    { + apply sound_comp with
          (dy1:=Phi_Par acts_eff1 acts_eff2) (dy2:=Phi_Par acts_mu1 acts_mu2).
  ...
  Case "set_ref e1 e2".
    ...
    eexists. split. unfold In, fold_subst_eps, Singleton_Static_Action, fold_subst_sa.
    eexists. intuition. 
    simpl. simpl in H0; inversion H0; subst. rewrite subst_rho_rgn_const. constructor.
  Case "nat_plus x y".
    assert (loc_TcVal : exists stty', 
             (forall l t', find_ST l stty = Some t' -> find_ST l stty' = Some t')
             /\ TcHeap (lheap, stty')
      ...
      by (eapply ty_sound; eauto).
    destruct loc_TcVal as [sttyx [Weakx [TcHeapx TcVal_x]]]; eauto.
    rewrite fold_dist_union.
  ...
  Case "eff_concat".
    assert (loc_TcVal : exists stty', 
             (forall l t', find_ST l stty = Some t' -> find_ST l stty' = Some t')
             /\ TcHeap (hp', stty')
             /\ TcVal (stty', Eff effa, subst_rho rho Ty_Effect))
      by (eapply ty_sound; eauto).
    destruct loc_TcVal as [sttyx [Weakx [TcHeapx TcVal_x]]]; eauto.
    rewrite fold_dist_union.
    apply sound_comp; eauto.
