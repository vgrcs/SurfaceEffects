...
Proof.
  ...
  remember (InitialState heap env rho (EPlus e1 e2))
    as start eqn:HStart.
...
Proof.
  intros heap env rho e1 e2 phi heap_final v_final HSteps.
  remember (InitialState heap env rho (EMinus e1 e2))
    as start eqn:HStart.
  remember (StDone heap_final v_final) as final eqn:HFinal.
  destruct HSteps as [state | state label state' phi0 state'' HStep HTail].
  - rewrite HStart in HFinal. inversion HFinal.
...
Proof.
  intros heap env rho e1 e2 phi heap_final v_final HSteps.
  remember (InitialState heap env rho (ETimes e1 e2))
    as start eqn:HStart.
  remember (StDone heap_final v_final) as final eqn:HFinal.
  destruct HSteps as [state | state label state' phi0 state'' HStep HTail].
  - rewrite HStart in HFinal. inversion HFinal.
...
Proof.
  intros heap env rho e1 e2 phi heap_final v_final HSteps.
  remember (InitialState heap env rho (EEq e1 e2))
    as start eqn:HStart.
  remember (StDone heap_final v_final) as final eqn:HFinal.
  destruct HSteps as [state | state label state' phi0 state'' HStep HTail].
  - rewrite HStart in HFinal. inversion HFinal.
