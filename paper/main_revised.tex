...
views are introduced later as proof-facing views over this list trace. Static
...
\begin{itemize}
  ...
  \item a checked pair-parallel interface based on a decidable computed-effect
        ...
        branch surface effects to deterministic terminal executions;
...
parallel composition when a theorem needs to remember proof-facing trace
structure.
...
On static effects, the write-freedom side condition rules out writes, while the
separate no-allocation side condition rules out allocation. Their conjunction is
the proof-side predicate $\HeapNeutral$: a heap-neutral static effect has neither
allocation nor write actions. Ordinary static-effect soundness is an auxiliary
theorem about evaluation traces: if the static erasure $\staticset{\phi}$ of a
trace is contained in a write-free static effect, then the trace contains no
writes; if it is contained in a heap-neutral static effect, then the trace has no
allocations and no writes. This is separate from the computed-effect precision
bridge $\phi\approxx\Theta$, which deliberately keeps concrete locations until
they are erased for static comparison. Consequently, heap-neutral surface-effect
...
\mathsf{init}(H,E,P,e)
  ...
  \mathsf{done}(H',v)\,;\,\Phi,
\]
where $\Phi$ is a structured trace view, and the surface-effect expression terminates
with computed effect $\Theta$ and structured trace view $\Phi_s$, then
\[
...
surface-effect correctness. In the current mechanization, trace-view coverage
is defined by flattening the view to its underlying machine trace; future
scheduler-sensitive semantics can strengthen this interface by preserving
parallel trace structure directly.
