...
\newcommand{\ptuple}[2]{\mathopen{\llparenthesis}\,#1,\;#2\,\mathclose{\rrparenthesis}} % checked parallel tuple
...
yields a \emph{computed effect}, which is the run-time summary value inspected
by the scheduler. The contribution is not merely that effects appear in types,
but that a computation can be paired with another well-typed source term that
computes a summary of the computation before the computation itself is accepted
to checked parallel execution.
...
computation, but it gives up precision and therefore prevents the checked-
parallel rule from being applied. Between these extremes, surface-effect terms
...
actual executions.  The table writes $\otimes$ for the relevant conflict
comparison at each level; the formal checked-parallel rule later refines the
computed-effect comparison into the computed-effect disjointness and
non-conflict premises.
...
parallel computation. Unlike optimistic post-validation, the calculus does not
start a checked-parallel computation and then roll it back after discovering a
...
are aliases or two different addresses inside the same region. Consequently, a
parallel tuple such as \lstinline|incr x| and \lstinline|incr y| is rejected by a
coarse static effect check, even in executions where the two references point to
...
fun loop (xs : int ref at 7 list) : unit =
  ...
  | x :: [] => incr[7] x
  | x :: y :: zs =>
      let val () = (| { incr[7] x | incr_eff[7] x },
                      { incr[7] y | incr_eff[7] y } |)
      in loop zs end
\end{lstlisting}
...
$\widehat{\Read(r)} \oplus \widehat{\Write(r)}$ is evaluated before the corresponding call to
\lstinline|incr| is accepted for checked parallel execution. Its result is a
...
argument \lstinline|r|. The scheduler can then compare computed effects that
mention concrete region/location pairs rather than only the region $\rho$.
...
maximally conservative, so the checked-parallel rule would not be justified.
...
value-dependent result is computed at run time, immediately before the scheduling
...
effect that covers the trace of the paired computation.  The scheduler consumes
this bridge, together with computed-effect disjointness and non-conflict, to
justify checked parallel execution.

\paragraph{Computed-effect conflict and disjointness.}\label{sec:disjoint}
The scheduler guard is stated entirely over computed effects.  It requires the
...
$\lambda$-calculus with mutable references, recursive functions, and implicit
parallel tuples.  The distinctive syntactic form is the recursive abstraction
...
or a concrete region. Pair types classify the values returned by successful
parallel tuples. Function types record two latent effects: in
...
arithmetic and equality, heap operations, checked parallel tuples, and
surface-effect terms. In $\mu f.\lambda x.\{e_b\mid e_s\}$, the argument
...
The checked parallel tuple
$\ptuple{\efun{1}\,\earg{1}}{\efun{2}\,\earg{2}}$ pairs two computational applications
with the summaries obtained from their corresponding effect applications.  The
small-step operational semantics first evaluates the two summary applications to
computed effects.  Only after the computed-effect check succeeds does the
scheduler-facing semantics enable checked parallel execution of the two
computational applications.  This separation between syntax, ordinary
continuation-machine execution, and checked scheduled execution is important for
the metatheory in Sections~\ref{sec:opsem} and~\ref{sec:smallstep}.
...
summary. The tuple rule is shown in the canonical form used by the scheduled
checked theorem: the computational branch summaries are the corresponding
effect applications.
...
\inferrule*[lab=Eff-Par]
  {\Gamma;\Delta \vdash \eapp \efun{1}\,\earg{1} :
      \mathsf{eff}\,!\,\epsilon_{e1} \\
   \ReadOnly(\epsilon_{e1}[P]) \\
   \Gamma;\Delta \vdash \eapp \efun{2}\,\earg{2} :
      \mathsf{eff}\,!\,\epsilon_{e2} \\
   \ReadOnly(\epsilon_{e2}[P]) \\
   \Gamma;\Delta;P \vdash \eapp \efun{1}\,\earg{1} \desc \thetahat_1 \\
   \Gamma;\Delta;P \vdash \eapp \efun{2}\,\earg{2} \desc \thetahat_2 \\
   \Gamma;\Delta;P \vdash \capp \efun{1}\,\earg{1}
      \desc \eapp \efun{1}\,\earg{1} \\
   \Gamma;\Delta;P \vdash \capp \efun{2}\,\earg{2}
      \desc \eapp \efun{2}\,\earg{2}}
  {\Gamma;\Delta;P \vdash
   \ptuple{\capp \efun{1}\,\earg{1}}{\capp \efun{2}\,\earg{2}}
   \desc
     (\thetahat_1 \oplus \thetahat_2)
     \oplus
     ((\eapp \efun{1}\,\earg{1}) \oplus
      (\eapp \efun{2}\,\earg{2}))}
\end{mathpar}
...
reuse this approximation story while stating finite-prefix safety directly over
the continuation machine, together with checked scheduled execution and
explicit blocked guard states.
...
\noindent\emph{In words.} If each branch trace is covered by its computed
effect and the two computed effects are disjoint and nonconflicting, then the
parallel trace is deterministic.
...
The predicate $\mathsf{Agree}_{\parallel}(s)$ is vacuous for ordinary states
and says that the two branch heaps agree when $s$ is an active checked
parallel-run state.
For scheduled executions, we use the same $\Sigma\vdash_{\mathsf{tr}}\phi$
notation for direct typing of a structured trace.
The dynamic tuple guard is assumed decidable:
\[
\forall \Theta_1,\Theta_2.\;
(\Disjoint(\Theta_1,\Theta_2)\land\neg(\Theta_1\conflict\Theta_2))\lor
\neg(\Disjoint(\Theta_1,\Theta_2)\land\neg(\Theta_1\conflict\Theta_2)).
\]

\noindent\emph{In words.} Every finite execution prefix of a well-typed
initial state remains well typed, has a typed trace, and is either terminal,
able to step, or at an explicit dynamic-check boundary.
...
If
\[
\mathsf{init}(H,E,P,e) \longrightarrow^{*} s\,;\,\phi,
\]
...
\mathsf{NotStuck}(s)
\quad\land\quad
\mathsf{Agree}_{\parallel}(s)
\quad\land\quad
\Sigma' \vdash_{\mathsf{tr}} \phi.
\]
\end{theorem}

\noindent\emph{In words.} Every finite execution prefix of a well-typed expression reaches a well-typed, non-stuck state and produces a well-typed trace.
...
For checked parallel tuples, the small-step semantics exposes the dynamic
...
splits on whether $\Disjoint(\Theta_1,\Theta_2)\land\neg(\Theta_1\conflict\Theta_2)$ holds.
...
\noindent\emph{In words.} At a parallel-tuple check boundary, either the
computed summaries pass and checked branch execution is safe, or the check
fails at a distinguished blocked state that is not an ill-typed stuck state.

\begin{theorem}[Dynamic guard finite-prefix safety]
...
\Sigma;P\vdash E:\Gamma
\\[1mm]
\Gamma;\Delta\vdash \capp \efun{1}\,\earg{1}:\tau_1\,!\,\epsilon_1
\qquad
\Gamma;\Delta\vdash \capp \efun{2}\,\earg{2}:\tau_2\,!\,\epsilon_2
\\[1mm]
...
\[
\begin{array}{l}
\bigl(\Disjoint(\Theta_1,\Theta_2)\land\neg(\Theta_1\conflict\Theta_2)
...
\noindent\emph{In words.} If checked branch execution terminates, its final
heap, value, and trace satisfy the expected typing invariants; otherwise the
failed guard remains an explicit blocked outcome.

\begin{theorem}[Dynamic guard terminal soundness]
Under the same hypotheses, either the check succeeds and
\[
\begin{array}{l}
\forall \phi,H',v.\;
s_{\mathsf{chk}}\longrightarrow^{*}_{\parallel}\mathsf{done}(H',v)\,;\,\phi
...
\land \Sigma' \vdash_{\mathsf{tr}} \phi,
\end{array}
\]
...
\mathsf{init}(H,E,P,e)
  \longrightarrow^{*}_{\mathsf{sch}}
  \mathsf{done}(H_1,v_1)\,;\,\phi_1
\qquad\text{and}\qquad
\mathsf{init}(H,E,P,e)
  \longrightarrow^{*}_{\mathsf{sch}}
  \mathsf{done}(H_2,v_2)\,;\,\phi_2,
\]
then
\[
H_1 = H_2
...
The scheduled relation packages the tuple-specific operational shape
explicitly: summary evaluation precedes the checked branch phase, the branch
machine records left and right traces, and a successful computed-effect check
gives the disjointness needed for schedule independence. Coverage of each
branch trace by the computed effect that authorized it is proved separately
from the back-triangle relation. The theorem is quantified over arbitrary
...
\[
\begin{array}{c}
\mathsf{init}(H,E,P,\eapp \efun{1}\,\earg{1})
  \longrightarrow^{*}
  \mathsf{done}(H_{e1},\mathsf{eff}(\Theta_1))\,;\,\phi_{e1},
\\[1mm]
\mathsf{init}(H,E,P,\eapp \efun{2}\,\earg{2})
  \longrightarrow^{*}
  \mathsf{done}(H_{e2},\mathsf{eff}(\Theta_2))\,;\,\phi_{e2},
\end{array}
\]
and the dynamic check on $\Theta_1$ and $\Theta_2$ succeeds.  The summary traces
$\phi_{e1}$ and $\phi_{e2}$ are handled by read-only reasoning: they preserve
the heap used to start the checked computation branches.

The checked branch machine then runs the two computational applications and
records branch traces $\phi_{b1}$ and $\phi_{b2}$, together with a residual
scheduler trace $\phi_p$, so that the structured execution trace has the shape
\[
\phi =
  (\phi_{e1}\parallel\phi_{e2})\tracecat
  ((\phi_{b1}\parallel\phi_{b2})\tracecat\phi_p).
\]
Surface-effect correctness for the two branches provides the coverage evidence
\[
\phi_{b1}\approxx\Theta_1
\qquad\text{and}\qquad
\phi_{b2}\approxx\Theta_2.
\]
Thus the computed summaries cover the branch computation traces, while the
summary-evaluation traces are separately read-only and heap preserving.  This is
the shape consumed by scheduler soundness: the summary-evaluation traces remain
read-only side traces, and the branch-computation traces are the traces covered
by $\Theta_1$ and $\Theta_2$.

\begin{theorem}[Back-triangle checked tuple coverage]
Assume the back-triangle relation gives
\[
\ptuple{\capp \efun{1}\,\earg{1}}{\capp \efun{2}\,\earg{2}}
\desc
(\epsilon_1\oplus\epsilon_2)\oplus
(\eapp \efun{1}\,\earg{1}\oplus\eapp \efun{2}\,\earg{2}),
\]
where the two computation branches are related to their corresponding
summaries. If an initial scheduled checked tuple run terminates,
\[
\mathsf{init}\bigl(H,E,P,
  \ptuple{\capp \efun{1}\,\earg{1}}{\capp \efun{2}\,\earg{2}}\bigr)
  \longrightarrow^{*}_{\mathsf{sch}}
  \mathsf{done}(H',v)\,;\,\phi,
\]
then there exist summary traces $\phi_{e1},\phi_{e2}$, branch traces
$\phi_{b1},\phi_{b2}$, a residual checked trace $\phi_p$, and computed
summaries $\Theta_1,\Theta_2$ such that
\[
\phi=(\phi_{e1}\parallel\phi_{e2})\tracecat
  ((\phi_{b1}\parallel\phi_{b2})\tracecat\phi_p),
\qquad
\mathsf{OK}(\Theta_1,\Theta_2),
\qquad
\phi_{b1}\approxx\Theta_1,
\qquad
\phi_{b2}\approxx\Theta_2.
...
The theorem covers the branch computation traces.  The prefix
$\phi_{e1}\parallel\phi_{e2}$ records summary evaluation and is handled by
read-only heap-neutrality; the residual trace $\phi_p$ records the final
checked-machine return phase.

\begin{theorem}[Checked tuple replay]
Under the hypotheses of back-triangle checked tuple coverage, the checked
branch machine exposes the branch trace projection
\[
s_\parallel\longrightarrow^{*}_{\mathsf{br}}
s'\,;\,\phi_p;\,\phi_{b1};\,\phi_{b2}.
\]
The mechanized replay corollary records this projection together with the
terminal residual checked-machine run.
\[
\mathsf{OK}(\Theta_1,\Theta_2),
\qquad
\phi_{b1}\approxx\Theta_1,
...
\noindent\emph{Mechanized induction closure.}
The Coq wrapper for the previous theorem currently takes the standard
small-step correctness induction package as an explicit premise. This is not a
runtime certificate and it is not derived from traces; it is the remaining
proof-engineering step needed to turn the wrapper into a premise-free theorem.

This replay statement is again about the computation phase after the dynamic
check.  It does not replay the summary-evaluation prefix as part of the
computed effect; that prefix remains governed by the read-only heap-neutrality
lemma.
...
ordinary executions, checked scheduled executions, and explicit blocked guard
states; it also proves terminal type and trace soundness for completed
scheduled machine runs, and scheduler independence for terminal scheduled runs.
