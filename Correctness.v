    ...
    assert (clsTcVal : exists stty',  
      ...
      SCase "bacts ⊑ eff".     
        ...
        SSSCase "Mu_App ef ea0 << (efff0 ⊕ (effa0 ⊕ Eff_App ef ea0))".
          ...
          assert (H' : facts  ⊑ effa0) by
              (eapply IHBS1_1 with (ee:=a); eauto; inversion HRonly; subst; assumption).
...
Proof.
    ...
    inversion HBt as [ | | | |  
                       ...
                       | ? ? ? ? ? ? ? ? ? ?  TcExp_ef TcExp_ea HBt_ef HBt_ea HR_ef HR_ea  
                       | | | | | | | |]; subst; 
    inversion HEff; subst. (*try (inversion H11 inversion H20; subst).*)  
    ...
    SCase "Mu_App ef ea0 << (efff0 ⊕ (effa0 ⊕ Eff_App ef ea0))".  
       ...
       { inversion H11.
         eapply IHBS1_2 with (p':=phia0); eauto .
         - rewrite HEq_1. eassumption.
         - inversion HRonly; subst. inversion H22.  assumption.
         - rewrite HEq_1. assumption. }
       destruct RH2 as [h_eq_2 [v_eq_2 a_eq_2]].
       
       assert ( RH3 : H.Equal h' h'_ /\ v = v_ /\ bacts = bacts0).
       { inversion H11.
         eapply IHBS1_3 with (ee:=ee'0) (stty:=sttya) (eff:=effb0) (p':=phib0) (h'':=h''); eauto.
         - subst. assumption.
         - rewrite HEq_2 in BS1_2.
           rewrite HEq_1 in BS1_1. 
           rewrite HEq_1 in BS1_2.
           rewrite HEq_2. rewrite HEq_1. 
           rewrite <- H7.
           eapply DynamicConstraints; eauto.
         - subst. eapply ext_stores__bt; eauto.   
         - subst. inversion HRonly; inversion H4; assumption.
         - subst.
           { apply update_env; simpl.  
             - eapply ext_stores__env; eauto. 
               apply update_env.  
    ...
    SCase "Mu_App ef ea0 << (⊤)". 
        ...
        assert ( RH3 : H.Equal h' h'_ /\ v = v_ /\ bacts = bacts0).
        { eapply IHBS1_3 with (ee:=⊤) (stty:=sttya); eauto.
          - inversion HEff.
            econstructor.
          - econstructor.
          - { apply update_env; simpl.  
...
Proof.
  intros heap_a heap_b env rho exp heap1 heap2 val1 val2 acts1 acts2 Heq BS1.
  intros BS2.
  ...
  intros static ty eff; intros HSound rgns HRho ctxt stty HHeap HEnv HExp p; intros HRonly ee_exp HEff HBt;
  intros heap_b Heq heap2 val2 acts2 BS2;
  try (solve [inversion BS2; subst; intuition]).
  - inversion BS2; subst. intuition. rewrite H in H1. inversion H1; subst. reflexivity.
  - inversion HExp as  [ | | | | | 
                         ? ? ? ? ? ? ? ? ? ? ? HExp_ef HExp_ea 
                         | | | | | | | | | | | | | | | | | | ]; subst.
    assert (clsTcVal : exists stty',  
      ...
      assert ( RH1 : facts  ⊑ eff ->
                     ...
                     Cls (env', rho', Mu f x ec' ee') = Cls (env'0, rho'0, Mu f0 x0 ec'0 ee'0) /\ 
                     facts = facts0 ).  
      { intro. eapply IHBS1_1; eauto. 
        -  eapply ReadOnlyStaticImpliesReadOnlyPhi with (phi:=facts) in HR_ef. 
           + apply ReadOnlyTracePreservesHeap_1 in BS1_1.  
             * symmetry in BS1_1.
               inversion HEff; subst. reflexivity.  
             * assumption. 
           + assert (facts_Eff : Epsilon_Phi_Soundness (fold_subst_eps rho static_ef, facts)) by
                  (eapply eff_sound; eauto). 
             eassumption.
        - admit.
        - assert (heap_b = fheap0) by admit.
          rewrite <- H0 in H2.
          assumption. }   
          
      destruct RH1 as [h_eq_1 [v_eq_1 a_eq_1]]. 
      inversion HSound; subst. inversion H1; subst. assumption.
      inversion v_eq_1. subst.
      assert ( RH2 : aacts  ⊑ eff -> H.Equal aheap aheap0 /\ v = v0 /\ aacts = aacts0).  
      { intro. 
        eapply IHBS1_2; eauto. 
        - eapply ext_stores__env; eauto.
        - eapply ext_stores__exp; eauto.  
        - assert (heap_a = fheap) by admit.
          rewrite <- H0. eassumption.
        - eapply ext_stores__bt; eauto.   
          admit.
        - assert (fheap = fheap0) by admit.
          apply Equal_heap_equal.
          assumption. }      
      destruct RH2 as [h_eq_2 [v_eq_2 a_eq_2]]; subst. 
      inversion HSound; subst. inversion H1; subst. assumption.
      
      assert ( RH3 : bacts  ⊑ eff -> H.Equal heap1 heap2 /\ val1 = val2 /\ bacts = bacts0).
      { intro. eapply IHBS1_3; eauto. 
        - { apply update_env; simpl.  
                - eapply ext_stores__env; eauto. 
                  apply update_env.  
                  + eassumption.
